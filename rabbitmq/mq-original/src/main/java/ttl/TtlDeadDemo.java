 ...
 * note 也就是说这种消息，只有在队列头的时候才会去检查消息是否过期，如果后面的消息已经过期了，不能及时的丢弃到死信队列中，那么其实就是有问题的，时间不准确。
 * note 解决方案可以通过rabbitmq的插件实现，添加一种交换机类型：x-delayed-message
 * 队列设置消息TTL： 因为通过队列设置消息TTL，整个队列中的过期时间是一样的（不另外通过消息属性设置TTL），那么过期的消息势必出现在队列头一整段，这个时候从队列头扫描（定时扫描）到没有过期的消息前，将这一段的消息全部抛弃或者进入死信队列。
 * note 这种可以正常的原因是，队列中所有的过期时间都是一样的，那么队列头的消息是肯定大于后面的过期时间的（队列头先过期），就只需要从队列头扫描过期的消息即可
 */
...
public class TtlDeadDemo {
    public static void main(String[] args) throws Exception{
        ...
        // note 设置当前队列的死信交换机(也就是消息过期之后，自动发送的指定的交换机)
